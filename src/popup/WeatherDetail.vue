<template>
    <div>
        <div class="is-flex is-flex-direction-row is-flex-wrap-nowrap is-align-items-center is-justify-content-space-around">
            <div>
                <h4 class="subtitle is-7 has-text-centered m-0">{{displayDate(weatherData.dt)}}</h4>
                <ul class="level">
                    <li v-for="item in weatherData.weather" :key="item.id" class="level-item has-text-centered is-flex is-flex-direction-column">

                        <p class="heading"><img class="weather-icon" v-bind:src="'http://openweathermap.org/img/wn/'+item.icon+'@2x.png'" v-bind:title="item.description"></p>
                        <p class="subtitle is-6 nowrap">{{item.description}}</p>
            ...
            <div>
                <div class="is-flex is-flex-direction-row is-justify-content-center is-align-items-baseline">
                    <p class="title is-1 has-text-centered m-0">{{displayTemp(weatherData.main.temp)}}</p>
